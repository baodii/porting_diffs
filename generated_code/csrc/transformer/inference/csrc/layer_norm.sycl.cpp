...
/*
DPCT1110:45: The total declared local variable size in device function fused_ln exceeds 128 bytes
...
#pragma unRoll
    ...
    reduce::partitioned_block<rop::Add, threadsPerGroup>(tb, warp, sum, item_ct1, reduce_buffer);
...
#pragma unRoll
    ...
    reduce::partitioned_block<rop::Add, threadsPerGroup>(
        tb, warp, mean_diff, item_ct1, reduce_buffer);
    const float variance = mean_diff / elems_per_row;
    /*
    DPCT1013:51: The rounding mode could not be specified and the generated code may have different
...
/*
DPCT1049:46: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
      ...
      cgh.parallel_for(sycl::nd_range<3>(grid * block, block),                                    \
                       [=](sycl::nd_item<3> item_ct1) [[intel::reqd_sub_group_size(32)]] {        \
                         fused_ln<T, unRollFactor, threadsPerGroup, maxThreads>(                  \
                             ...
                             elems_per_row_ct5,                                                   \
                             item_ct1,                                                            \
                             reduce_buffer_acc_ct1.get_pointer());                                \
                       });                                                                        \
    });                                                                                           \
  }
...
template void launch_fused_ln(sycl::half*,
                              const sycl::half*,
                              const sycl::half*,
                              const sycl::half*,
                              float,
...
#endif
template void
launch_fused_ln(float*, const float*, const float*, const float*, float, int, int, dpct::queue_ptr);
...
/*
DPCT1110:47: The total declared local variable size in device function fused_residual_ln exceeds 128
...
#pragma unRoll
    ...
    const float variance = mean_diff / elems_per_row;
    /*
    DPCT1013:52: The rounding mode could not be specified and the generated code may have different
    accuracy than the original code. Verify the correctness. SYCL math built-in function rounding
    mode is aligned with OpenCL C 1.2 standard.
    */
    const float denom = sycl::rsqrt(variance + epsilon);
...
/*
DPCT1049:48: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
*/
#define LAUNCH_FUSED_RES_LN(unRollFactor, threadsPerGroup, maxThreads)                            \
  {                                                                                               \
    ...
    stream->submit([&](sycl::handler& cgh) {                                                      \
      ...
      auto vals_ct2 = vals;                                                                       \
      auto residual_ct3 = residual;                                                               \
      auto bias_ct4 = bias;                                                                       \
      auto gamma_ct5 = gamma;                                                                     \
      auto beta_ct6 = beta;                                                                       \
...
/*
DPCT1049:49: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
*/
#define LAUNCH_FUSED_RES_LN_STORE_PRE_LN_RES(unRollFactor, threadsPerGroup, maxThreads)           \
  {                                                                                               \
    ...
    stream->submit([&](sycl::handler& cgh) {                                                      \
      ...
      auto res_output_ct1 = res_output;                                                           \
      auto vals_ct2 = vals;                                                                       \
      auto residual_ct3 = residual;                                                               \
      auto bias_ct4 = bias;                                                                       \
      auto gamma_ct5 = gamma;                                                                     \
...
template void launch_fused_residual_ln(sycl::half*,
                                       const sycl::half*,
                                       const sycl::half*,
                                       const sycl::half*,
                                       const sycl::half*,
...
template void launch_fused_residual_ln_store_pre_ln_res(sycl::half*,
                                                        sycl::half*,
                                                        const sycl::half*,
                                                        const sycl::half*,
                                                        const sycl::half*,

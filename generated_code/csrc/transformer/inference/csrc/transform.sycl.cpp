...
template <typename T>
/*
DPCT1110:1: The total declared local variable size in device function bias_add_transform_0213
...
#pragma unroll
            for (int o = 0; o < 4; o++) {
                ...
                q_h[o].x() = conversion::to<T>(-1.0 * q_data[1] * sycl::sin(inv_freq) +
                                               q_data[0] * sycl::cos(inv_freq));
                q_h[o].y() = conversion::to<T>(q_data[0] * sycl::sin(inv_freq) +
...
template void launch_bias_add_transform_0213(sycl::half* output,
                                             sycl::half* k_cache,
                                             sycl::half* v_cache,
                                   ...
                                   dpct::queue_ptr stream)
{
    ...
    /*
    DPCT1049:4: The work-group size passed to the SYCL kernel may exceed the limit. To get the
    device limit, query info::device::max_work_group_size. Adjust the work-group size if needed.
    */
    {
        dpct::has_capability_or_fail(stream->get_device(),
...
template void launch_pad_add_transform_0213(sycl::half* output,
                                            const sycl::half* vals,
                                            int batch_size,
                                            int hidden_dim,
                                            int seq_length,
...
template <typename T>
/*
DPCT1110:5: The total declared local variable size in device function bias_add_transform_0213_v2
exceeds 128 bytes and may cause high register pressure. Consult with your hardware vendor to find
the total register size available and adjust the code, or use smaller sub-group size to avoid high
register pressure.
*/
...
template void bias_add_transform_0213_v2(sycl::half* output,
                                         const sycl::half* vals,
                                         const sycl::half* bias,
                                         int hidden_dim,
                                         int seq_length,
...
void transform4d_0213_v2(T* out, const T* in, int heads, int seq_length, int hidden_dim,
                         const sycl::nd_item<3> &item_ct1, sycl::float4 *in_data)
{
    int d0_stride = hidden_dim * seq_length;
    int d1_stride = hidden_dim;
    int d2_stride = hidden_dim / heads;
...
template void transform4d_0213_v2(sycl::half* out,
                                  const sycl::half* in,
                                  int heads,
                                  int seq_length,
                                  int hidden_dim,
    ...
    */
    {
        ...
        stream->parallel_for(
            sycl::nd_range<3>(grid_dims * block_dims, block_dims), [=](sycl::nd_item<3> item_ct1) {
                transform4d_0213(out, in, heads, seq_length, hidden_dim, head_ext, item_ct1);
            });
    }
}
...
template void launch_transform4d_0213(sycl::half* out,
                                      const sycl::half* in,
                                      int batch_size,
                                      int heads,
                                      int seq_length,

...
{
    /*
    DPCT1010:13: SYCL uses exceptions to report errors and does not use the error codes. The call
    was replaced with 0. You need to rewrite this code.
    */
    dpct::err0 err = 0;
    if (err == 0) return;
    /*
    DPCT1009:14: SYCL uses exceptions to report errors and does not use the error codes. The
...
/*
DPCT1110:1: The total declared local variable size in device function attn_softmax_v2 exceeds 128
                     ...
                     float* partialSum)
{
    ...
    if (iter_offset < total_count) {
        ...
        for (int i = 1; i < WARP_SIZE; i *= 2) {
            auto temp = sycl::permute_group_by_xor(item_ct1.get_sub_group(), max_val, i);
...
/*
DPCT1110:6: The total declared local variable size in device function attn_softmax_v2 exceeds 128
bytes and may cause high register pressure. Consult with your hardware vendor to find the total
register size available and adjust the code, or use smaller sub-group size to avoid high register
pressure.
*/
...
/*
DPCT1049:11: The work-group size passed to the SYCL kernel may exceed the limit. To get the device
...
#define LAUNCH_ATTN_SOFTMAX_V2(iterations)                                                         \
  {                                                                                                \
    ...
    stream->submit([&](sycl::handler& cgh) {                                                       \
      /*                                                                                           \
      DPCT1101:15: 'MAX_WARP_NUM' expression was replaced with a value. Modify the code to         \
      use the original expression, provided in comments, if it is correct.                         \
      */                                                                                           \
      sycl::local_accessor<float, 1> partialSum_acc_ct1(sycl::range<1>(32 /*MAX_WARP_NUM*/), cgh); \
                                                                                                   \
      auto vals_ct0 = vals;                                                                        \
      auto mask_ct1 = mask;                                                                        \
      auto alibi_ct2 = alibi;                                                                      \
...
template void launch_attn_softmax_v2(sycl::half* vals,
                                     sycl::half* mask,
                                     sycl::half* alibi,
                                     float layer_scale,
                                     bool triangular,
...
#define DEF_ATTN_SOFTMAX_V2_BF16(_iter)                                                   \
    template void attn_softmax_v2<__nv_bfloat16, _iter>(__nv_bfloat16 * vals,  \
                                                                   __nv_bfloat16 * mask,  \
                                                                   __nv_bfloat16 * alibi, \
                                                                   float layer_scale,     \
                                                                   bool triangular,       \

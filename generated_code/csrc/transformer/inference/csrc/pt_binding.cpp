...
at::Tensor ds_bias_geglu(at::Tensor& activation, at::Tensor& bias)
{
    ...
    */
    ...
    if (activation.options().dtype() == torch::kFloat32) {
        ...
        launch_fused_bias_geglu((sycl::half*)output.data_ptr(),
                                (const sycl::half*)activation.data_ptr(),
                                (const sycl::half*)bias.data_ptr(),
                                rows,
                                channels,
                                InferenceContext::Instance().GetCurrentStream());
    }
...
at::Tensor ds_layer_norm(at::Tensor& input, at::Tensor& gamma, at::Tensor& beta, float epsilon)
{
    ...
    if (input.options().dtype() == torch::kFloat16) {
        launch_fused_ln((sycl::half*)output.data_ptr(),
                        (const sycl::half*)input.data_ptr(),
                        (const sycl::half*)gamma.data_ptr(),
                        (const sycl::half*)beta.data_ptr(),
                        epsilon,
                        rows,
                        elems_per_row,
                        InferenceContext::Instance().GetCurrentStream());
                                  ...
                                  float epsilon)
{
    ...
    if (input.options().dtype() == torch::kFloat16) {
        launch_fused_residual_ln((sycl::half*)output.data_ptr(),
                                 (const sycl::half*)input.data_ptr(),
                                 (const sycl::half*)residual.data_ptr(),
                                 (const sycl::half*)bias.data_ptr(),
                                 (const sycl::half*)gamma.data_ptr(),
                                 (const sycl::half*)beta.data_ptr(),
                                 epsilon,
                                                                ...
                                                                float epsilon)
{
    ...
    if (input.options().dtype() == torch::kFloat16) {
        launch_fused_residual_ln_store_pre_ln_res((sycl::half*)norm_output.data_ptr(),
                                                  (sycl::half*)res_output.data_ptr(),
                                                  (const sycl::half*)input.data_ptr(),
                                                  (const sycl::half*)residual.data_ptr(),
                                                  (const sycl::half*)bias.data_ptr(),
                                                  (const sycl::half*)gamma.data_ptr(),

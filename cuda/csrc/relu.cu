...
*/
template <typename T>
__global__ void fused_bias_relu(T *input, const T *bias, int total_count,
...
template <typename T>
void launch_bias_relu(T *input, const T *bias, int intermediate_size,
...
template void launch_bias_relu<float>(float *, const float *, int, int,
                                      cudaStream_t);
...
template void launch_bias_relu<__nv_bfloat16>(__nv_bfloat16 *,
                                              ...
                                              cudaStream_t);
#endif
template void launch_bias_relu<__half>(__half *, const __half *, int, int,
                                       cudaStream_t);

...
__global__ void bias_add_transform_0213(float *output, float *k_cache,
                                        float *v_cache, const float *vals,
                                        const float *bias, int hidden_dim,
                                        int seq_length, unsigned seq_offset,
                                        int heads, int rotary_dim,
                                        bool rotate_half, bool rotate_every_two,
                                        int head_ext, int max_out_tokens) {
  ...
  int d2 =
      threadIdx.y + (blockIdx.z % head_ext) * (heads / head_ext); // Head (0-11)
...
#pragma unroll
      for (int o = 0; o < 2; o++) {
        ...
        q_f[o].x =
            (-1.0 * q_f[o].y * sinf(inv_freq) + q_f[o].x * cosf(inv_freq));
...
template <typename T>
__global__ void
bias_add_transform_0213(T *output, // q
                        T *k_cache, T *v_cache,
                        const T *vals, // qkv
                        const T *bias, int hidden_dim, int seq_length,
                        unsigned seq_offset, int all_tokens, int heads,
...
#pragma unroll
      for (int o = 0; o < 4; o++) {
        ...
        q_h[o].y = conversion::to<T>(q_data[0] * sinf(inv_freq) +
                                     q_data[1] * cosf(inv_freq));
      }
    }
    output_vec[d3] = q;
  } else
...
void launch_bias_add_transform_0213<float>(
    float *output, float *k_cache, float *v_cache, const float *vals,
    const float *bias, int batch_size, int seq_length, unsigned seq_offset,
    int all_tokens, int hidden_dim, int heads, int rotary_dim, bool rotate_half,
    bool rotate_every_two, cudaStream_t stream, int trans_count,
    int max_out_tokens) {
  hidden_dim >>= 2;
  ...
  bias_add_transform_0213<<<grid_dim, block_dim, 0, stream>>>(
      ...
      heads, rotary_dim >> 2, rotate_half, rotate_every_two, head_ext,
      max_out_tokens);
}

template <typename T>
void launch_bias_add_transform_0213(T *output, T *k_cache, T *v_cache,
                                    const T *vals, const T *bias,
                                    int batch_size, int seq_length,
                                    unsigned seq_offset, int all_tokens,
                                    int hidden_dim, int heads, int rotary_dim,
                                    bool rotate_half, bool rotate_every_two,
  ...
  bias_add_transform_0213<<<grid_dim, block_dim, 0, stream>>>(
      output, k_cache, v_cache, vals, bias, hidden_dim, seq_length, seq_offset,
      all_tokens, heads, rotary_dim >> 3, rotate_half, rotate_every_two,
      head_ext, max_out_tokens);
}

#ifdef BF16_AVAILABLE
template void launch_bias_add_transform_0213(
    __nv_bfloat16 *output, __nv_bfloat16 *k_cache, __nv_bfloat16 *v_cache,
    const __nv_bfloat16 *vals, const __nv_bfloat16 *bias, int batch_size,
    int seq_length, unsigned seq_offset, int all_tokens, int hidden_dim,
    int heads, int rotary_dim, bool rotate_half, bool rotate_every_two,
    cudaStream_t stream, int trans_count, int max_out_tokens);
#endif

template void launch_bias_add_transform_0213(
    __half *output, __half *k_cache, __half *v_cache, const __half *vals,
    const __half *bias, int batch_size, int seq_length, unsigned seq_offset,
    int all_tokens, int hidden_dim, int heads, int rotary_dim, bool rotate_half,
    bool rotate_every_two, cudaStream_t stream, int trans_count,
    int max_out_tokens);
...
__global__ void pad_add_transform_0213(float *output, const float *vals,
                                       int hidden_dim, int seq_length,
                                       int padded_seq_len, int heads,
                                       int padded_head_size) {}

template <typename T>
__global__ void pad_add_transform_0213(T *output, const T *vals, int hidden_dim,
                                       int seq_length, int padded_seq_len,
                                       int heads, int padded_head_size) {
  using T2 = typename std::conditional<std::is_same<T, __half>::value, __half2,
                                       __nv_bfloat162>::type;
  float4 ZERO;
...
// [B S C*H] - > C * [B A S N]
template <>
void launch_pad_add_transform_0213<float>(float *output, const float *vals,
...
template <typename T>
void launch_pad_add_transform_0213(T *output, const T *vals, int batch_size,
                                   int hidden_dim, int seq_length,
                                   int padded_seq_len, int heads,
                                   int padded_head_size, cudaStream_t stream) {
  hidden_dim >>= 3;
  ...
  pad_add_transform_0213<<<grid_dim, block_dim, 0, stream>>>(
      output, vals, hidden_dim, seq_length, padded_seq_len, heads,
      padded_head_size >> 3);
}
...
template void launch_pad_add_transform_0213(__half *output, const __half *vals,
                                            int batch_size, int hidden_dim,
                                            int seq_length, int padded_seq_len,
                                            int heads, int padded_head_size,
                                            cudaStream_t stream);
...
// Bias add
template <typename T>
__global__ void bias_add_transform_0213(T *output, const T *vals, const T *bias,
                                        int hidden_dim, int seq_length,
                                        int heads, int head_ext);

template <>
__global__ void bias_add_transform_0213<float>(float *output, const float *vals,
                                               const float *bias,
                                               int hidden_dim, int seq_length,
                                               int heads, int head_ext) {
  int d0_stride = hidden_dim * seq_length;
  ...
  float4 inputs =
      vals_vec[d0 * d0_stride * (gridDim.z / head_ext) + cnt * d1_stride +
  ...
  output_vec[cnt * d0_out_stride * gridDim.x + d0 * d0_out_stride +
...
template <typename T>
__global__ void bias_add_transform_0213_v2(T *output, const T *vals,
                                           const T *bias, int hidden_dim,
                                           int seq_length, int heads) {
  using T2 = typename std::conditional<std::is_same<T, __half>::value, __half2,
                                       __nv_bfloat162>::type;
...
template <typename T>
__global__ void transform4d_0213(T *out, const T *in, int heads, int seq_length,
                                 int hidden_dim, int head_ext);

template <>
__global__ void transform4d_0213<float>(float *out, const float *in, int heads,
  ...
  if (d2 < seq_length) {
    ...
    float4 vals_vec = in_vec[cnt * d0_stride * gridDim.x + d0 * d0_stride +
                             d1 * d1_stride + d2 * d2_stride + d3];
    out_vec[d0 * d0_out_stride * gridDim.z + cnt * d2_out_stride +
            d1 * d1_out_stride + d2 * d2_out_stride * gridDim.z + d3] =
        vals_vec;
  }
}
...
template <typename T>
__global__ void transform4d_0213_v2(T *out, const T *in, int heads,
                                    int seq_length, int hidden_dim) {
  __shared__ float4 in_data[3072];
...
#pragma unroll
  for (int iter = 0; iter < 2; iter++) {
    ...
    in_data[d3 + iter_offset +
            (iter_row / blockDim.y + (d1 % 2) * blockDim.z) * d1_stride] =
  ...
  int output_offset =
      d0 * d0_stride * blockDim.z + d2 * (iteration_stride << 1);
...
template __global__ void transform4d_0213_v2(__nv_bfloat16 *out,
                                             const __nv_bfloat16 *in, int heads,
                                             int seq_length, int hidden_dim);
#endif

template __global__ void transform4d_0213_v2(__half *out, const __half *in,
                                             int heads, int seq_length,
                                             int hidden_dim);

// 3 * [B A S N] - > [B S C*H]
template <>
void launch_transform4d_0213<float>(float *out, const float *in, int batch_size,
                                    int heads, int seq_length, int hidden_dim,
                                    cudaStream_t stream, int trans_count) {
  hidden_dim >>= 2;
  dim3 grid_dims(batch_size, heads * ((seq_length - 1) / 8 + 1), trans_count);
  dim3 block_dims(hidden_dim / heads, 8);
  transform4d_0213<float><<<grid_dims, block_dims, 0, stream>>>(
      out, in, heads, seq_length, hidden_dim, 1);
}

template <typename T>
void launch_transform4d_0213<T>(T *out, const T *in, int batch_size, int heads,
                                int seq_length, int hidden_dim,
                                cudaStream_t stream, int trans_count) {
  hidden_dim >>= 3;
  int head_ext = (hidden_dim - 1) / MAX_THREADS + 1;
...
template void launch_transform4d_0213(__nv_bfloat16 *out,
                                      const __nv_bfloat16 *in, int batch_size,
                                      int heads, int seq_length, int hidden_dim,
                                      cudaStream_t stream, int trans_count);
#endif

template void launch_transform4d_0213(__half *out, const __half *in,
                                      int batch_size, int heads, int seq_length,

...
struct hash_pair {
  ...
  std::size_t operator()(const std::pair<T1, T2> &pair) const {
    return hash_combine(std::hash<T1>()(pair.first),
                        std::hash<T2>()(pair.second));
  }
};

// DNNL engine and stream should be permnantly existing and binding to sycl
// queue
static std::pair<dnnl::engine, dnnl::stream>
get_dnnl_engine_stream(sycl::queue queue) {
...
inline int onednn_matmul(sycl::queue handle, bool trans_src, bool trans_wgt,
                         int m, int n, int k, const float alpha,
                         const float beta, const T *src_ptr, const T *wgt_ptr,
                         T *dst_ptr, int batch) {
  /*
  ...
  constexpr auto dnnl_dtype_16 = std::is_same<T, fp16>::value
  ...
  if constexpr (bmm) {
    src_md = dnnl::memory::desc(src_dims, dnnl_dtype_16,
                                trans_src ? dnnl::memory::format_tag::acb
                                          : dnnl::memory::format_tag::abc);
    wgt_md = dnnl::memory::desc(wgt_dims, dnnl_dtype_16,
                                trans_wgt ? dnnl::memory::format_tag::acb
                                          : dnnl::memory::format_tag::abc);
    dst_md = dnnl::memory::desc(dst_dims, dnnl_dtype_16,
                                dnnl::memory::format_tag::abc);
  } else {
    src_md = dnnl::memory::desc(src_dims, dnnl_dtype_16,
                                trans_src ? dnnl::memory::format_tag::ba
                                          : dnnl::memory::format_tag::ab);
    wgt_md = dnnl::memory::desc(wgt_dims, dnnl_dtype_16,
                                trans_wgt ? dnnl::memory::format_tag::ba
                                          : dnnl::memory::format_tag::ab);
    dst_md = dnnl::memory::desc(dst_dims, dnnl_dtype_16,
                                dnnl::memory::format_tag::ab);
  }
  ...
  if (alpha != 1.0f) {
    ...
    dnnl::memory alpha_mem({{1}, dnnl::memory::data_type::f32, {1}}, engine,
                           &alpha_v);
  ...
  auto matmul_pd =
      dnnl::matmul::primitive_desc(engine, src_md, wgt_md, dst_md, attr);
  ...
  auto scratchpad_tensor =
      at::empty({(int64_t)scratchpad_md.get_size()}, options);
...
template <typename T>
int onednn_matmul_ex(sycl::queue handle, bool trans_src, bool trans_wgt, int m,
                     int n, int k, const float alpha, const float beta,
                     const T *src_ptr, const T *wgt_ptr, T *dst_ptr) {
  return onednn_matmul<T, false>(handle, trans_src, trans_wgt, m, n, k, alpha,
                                 beta, src_ptr, wgt_ptr, dst_ptr, 1);
}

template <typename T>
int onednn_batchgemm(sycl::queue handle, int m, int n, int k, const float alpha,
                     const float beta, const T *src_ptr, const T *wgt_ptr,
                     T *dst_ptr, bool trans_src, bool trans_wgt, int batch) {
  return onednn_matmul<T, true>(handle, trans_src, trans_wgt, m, n, k, alpha,
                                beta, src_ptr, wgt_ptr, dst_ptr, batch);
}

template int onednn_matmul_ex(sycl::queue handle, bool trans_src,
                              bool trans_wgt, int m, int n, int k,
                              const float alpha, const float beta,
                              const bf16 *src_ptr, const bf16 *wgt_ptr,
                              bf16 *dst_ptr);

template int onednn_matmul_ex(sycl::queue handle, bool trans_src,
                              bool trans_wgt, int m, int n, int k,
                              const float alpha, const float beta,
                              const fp16 *src_ptr, const fp16 *wgt_ptr,
                              fp16 *dst_ptr);

template int onednn_batchgemm(sycl::queue handle, int m, int n, int k,
                              const float alpha, const float beta,
                              const bf16 *src_ptr, const bf16 *wgt_ptr,
                              bf16 *dst_ptr, bool trans_src, bool trans_wgt,
                              int batch);

template int onednn_batchgemm(sycl::queue handle, int m, int n, int k,
                              const float alpha, const float beta,
                              const fp16 *src_ptr, const fp16 *wgt_ptr,
                              fp16 *dst_ptr, bool trans_src, bool trans_wgt,
                              int batch);
